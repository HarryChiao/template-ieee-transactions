...
We calculate this task duration as the $cwnd$ elapsed time i.e., $t_{elapsed} = t_{j+\Delta} - t_{j}$. We then calculate the goodput ($g_{i, t_j}$) and packet losses during the estimation task, where $g_{i, t_j} = \frac{B_i(t_{j+\Delta})-B_i(t_j)}{t_{elapsed}}$. and $\ell_{i,t_j} = L_i(t_{j+\Delta})-L_i(t_j)$. We then form a state message according to $s_{i,t_j}=S(o_{i,t_{j+\Delta}},o_{i,t_{j}};\tau_{i,t_j})$, where the generation process is defined as follows:
$$ S(o_{i,t_{j+\Delta}},o_{i,t_{j}};\tau_{i,t_j}) \triangleq \left[g_{i, t_j}, \ell_{i,t_j}, t_{elapsed}\right]$$
The Receiver Measurement and State Generation process can be seen in Algorithm~\ref{alg:per_packet_state}.

\begin{figure}[!t]
\centering
\includegraphics[width=0.9\linewidth]{./fig/ReceiverMeasurementModule2_2.png}
...
\begin{algorithmic}[1]
\Require Control Message $M$, Replay Buffer $B$, Flow State Buffer $B_i$ for flow $i$
\Ensure 
...
Then the enforced $cwnd$ will be included into the ACK header and take effect at the sender side. For the receiver, before the next action request tick, the action will keep effective.
