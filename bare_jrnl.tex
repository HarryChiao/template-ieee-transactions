...
To address the limitation above, we derive a receiver-side congestion extent metric based on the $cwnd$ elapsed time. We extend the sender TCP feature to include its $cwnd$ in TCP header. 
...
The eventual observation is encapsulated into a State Message and sent to the Window Decision Module for storage. The consecutive process can be seen in Algorithm~\ref{alg:per_packet_state}.
